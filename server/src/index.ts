...
const initializeDatabase = async () => {
  ...
  // Create Table (Keep province column in DB for compatibility, but don't use it in API)
...
app.post('/api/register', async (req: Request, res: Response) => {
  ...
  if (!fullname || !role || !organization || !phone_number || !email_address || !gender) {
...
app.put('/api/register/:id', async (req, res) => {
  ...
  const { fullname, role, organization, phone_number, email_address, gender } = req.body;
  try {
    const query = 'UPDATE registrations SET fullname=$1, role=$2, organization=$3, phone_number=$4, email_address=$5, gender=$6 WHERE id=$7';
    const params = [fullname, role, organization, phone_number, email_address, gender, id];
...
app.get('/api/export/csv', async (req, res) => {
  try {
    ...
    result.rows.forEach((r: any) => {
      csv += `${r.id},${escapeCSV(r.fullname)},${escapeCSV(r.role)},${escapeCSV(r.organization)},${escapeCSV(r.phone_number)},${escapeCSV(r.email_address)},${escapeCSV(r.gender)},${r.created_at}\n`;
    });
